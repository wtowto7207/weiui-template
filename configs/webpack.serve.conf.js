...
const postMessageToOpenPage = (entry) => {
    ...
    openpage += `&socket_host=${config.dev.host}`;
    openpage += `&socket_port=${config.dev.port + 1}`;
    return openpage;
};
        ...
        if (err) {
            ...
            devWebpackConfig.plugins.push(new FriendlyErrorsPlugin({
                compilationSuccessInfo: {
                    messages: [
                        `Your application is running here: ${chalk.blue.underline(`http://${devWebpackConfig.devServer.host}:${port}`)}.`
                    ],
                },
