package cc.weiui.framework.extend.view.tablayout;
...
import cc.weiui.framework.extend.view.tablayout.utils.UnreadMsgUtils;
import cc.weiui.framework.extend.view.tablayout.widget.MsgView;
...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private void addTab(final int position, View tabView) {
        ...
        IconTextView iv_tab_icontext = (IconTextView) tabView.findViewById(R.id.iv_tab_icontext);
        setImageView(false, mTabEntitys.get(position), iv_tab_icon, iv_tab_icontext);
...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
            ...
            if (mIconVisible) {
                ...
                if (isFontIcon(iconUrl)) {
                    ...
                    iv_tab_icontext.setVisibility(View.GONE);
                    iv_tab_icon.setVisibility(View.VISIBLE);
                }
                setImageView(isCurrent, tabEntity, iv_tab_icon, iv_tab_icontext);
                ...
                iv_tab_icontext.setLayoutParams(lp);
            } else {
                iv_tab_icon.setVisibility(View.GONE);
                iv_tab_icontext.setVisibility(View.GONE);
            }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            IconTextView iv_tab_icontext = (IconTextView) tabView.findViewById(R.id.iv_tab_icontext);
            CustomTabEntity tabEntity = mTabEntitys.get(i);
            setImageView(isSelect, tabEntity, iv_tab_icon, iv_tab_icontext);
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    public IconTextView getIcontextView(int tab) {
        View tabView = mTabsContainer.getChildAt(tab);
        IconTextView iv_tab_icontext = (IconTextView) tabView.findViewById(R.id.iv_tab_icontext);
        return iv_tab_icontext;
    }
    ...
    public void setMsgMargin(int position, float leftPadding, float bottomPadding) {
        ...
        if (tipView != null) {
            ...
            if (mIconVisible) {
                if (iconH <= 0) {
                    iconH = getImageViewIntrinsicHeight(mTabEntitys.get(position));
                }
                margin = mIconMargin;
            }
    ...
    private Map<String, Integer> imageViewHeight = new HashMap<>();

    @SuppressLint("SetTextI18n")
    private void setImageView(boolean isCurrent, CustomTabEntity tabEntity, ImageView icon, IconTextView icontext) {
        String iconUrl;
        int iconResid;
        RequestOptions myOptions = new RequestOptions().diskCacheStrategy(DiskCacheStrategy.ALL);
        if (isCurrent) {
            iconUrl = tabEntity.getTabSelectedIconUrl();
            iconResid = tabEntity.getTabSelectedIcon();
        } else {
            iconUrl = tabEntity.getTabUnselectedIconUrl();
        ...
        if (iconUrl != null) {
            ...
            Glide.with(icon.getContext()).load(iconUrl).apply(myOptions).listener(new RequestListener<Drawable>() {
                ...
                public boolean onResourceReady(Drawable resource, Object model, Target<Drawable> target, DataSource dataSource, boolean isFirstResource) {
                    imageViewHeight.put(finalIconUrl, resource.getIntrinsicHeight());
                    return false;
                }
            }).into(icon);
            return;
