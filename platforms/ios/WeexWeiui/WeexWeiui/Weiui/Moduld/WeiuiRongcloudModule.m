...
#import "WeiuiRongcloudModule.h"
#import <RongIMLib/RongIMLib.h>
#import <CommonCrypto/CommonDigest.h>
#import "WeexSDKManager.h"

@interface WeiuiRongcloudModule () <RCIMClientReceiveMessageDelegate, RCConnectionStatusChangeDelegate>
...
@implementation WeiuiRongcloudModule
...
- (void)login:(id)params callback:(WXModuleKeepAliveCallback)callback
{
    ...
    NSString *unionString = [NSString stringWithFormat:@"%@%@%ld", [WeexSDKManager sharedIntstance].rongSec, nonce, timestamp];
    ...
    [request setValue:[WeexSDKManager sharedIntstance].rongKey forHTTPHeaderField:@"App-Key"];
...
{
    NSLog(@"RCConnectionStatus = %d", status);
...
- (void)joinChatRoom:(id)roomId defMessageCount:(NSInteger)defMessageCount callback:(WXModuleKeepAliveCallback)callback
{
    ...
    [[RCIMClient sharedRCIMClient] joinChatRoom:self.roomID messageCount:defMessageCount success:^{
...
- (void)sendTextMessage:(NSString*)text callback:(WXModuleKeepAliveCallback)callback
{
    ...
    __weak typeof(WeiuiRongcloudModule) *ws = self;
