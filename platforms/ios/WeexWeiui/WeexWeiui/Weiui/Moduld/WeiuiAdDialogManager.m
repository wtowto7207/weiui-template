...
@interface WeiuiAdDialogView : UIControl
...
@implementation WeiuiAdDialogView
...
@implementation WeiuiAdDialogManager
...
- (void)loadAdDialogView:(UIImage*)img params:(id)params callback:(WXModuleKeepAliveCallback)callback
{
    ...
    WeiuiAdDialogView *adDialogView = [[WeiuiAdDialogView alloc] initWithFrame:window.bounds];
...
- (void)adDialogClose:(NSString*)dialogName
{
    if (dialogName.length > 0) {
        WeiuiAdDialogView *view = [self.adDialogDic objectForKey:dialogName];
        if (view) {
            [view colseAdDialog];
    ...
    if (_adDialogList.count > 0) {
        WeiuiAdDialogView *view = [_adDialogList firstObject];
        if (view) {
            [view colseAdDialog];
            [_adDialogList removeObject:view];
            [view removeFromSuperview];
