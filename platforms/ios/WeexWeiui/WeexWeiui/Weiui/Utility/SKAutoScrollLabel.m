...
static const CGFloat kDefaultAutoScrollInterval = 0.0f;
...
- (void)didMoveToWindow
{
    ...
    if (self.window) {
         [[NSNotificationCenter defaultCenter] addObserver:self    selector:@selector(scrollLabelIfNeeded)    name:UIApplicationDidBecomeActiveNotification object:nil];
...
- (void)scrollLabelIfNeeded
{
    ...
    // 滚动动画
    [UIView animateWithDuration:duration delay:self.autoScrollInterval options:UIViewAnimationOptionCurveLinear|UIViewAnimationOptionAllowUserInteraction animations:^{
...
//    if (!_scrolling) {
        // 时间转换
        CFTimeInterval pauseTime = self.scrollView.layer.timeOffset;
        // 计算暂停时间
        CFTimeInterval timeSincePause = CACurrentMediaTime() - pauseTime;
        // 取消
        self.scrollView.layer.timeOffset = 0;
        // local time相对于parent time世界的beginTime
        self.scrollView.layer.beginTime = timeSincePause;
        // 继续
        self.scrollView.layer.speed = 1;
//    }
...
- (void)stopScroll
{
    ...
    CFTimeInterval pauseTime = [self.scrollView.layer convertTime:CACurrentMediaTime() fromLayer:nil];
    
    // 设置layer的timeOffset, 在继续操作也会使用到
    self.scrollView.layer.timeOffset = pauseTime;
